...
from src.fetchers.hh_web import HH_VACANCIES_URL, fetch_hh_vacancies
...
async def test_fetch_hh_vacancies_success(hh_mock_data):
    ...
    response_text = await fetch_hh_vacancies(client, query_text=query_text)

    assert isinstance(response_text, str)
    assert response_text == hh_mock_data
